...
- [x] **SETTINGS-01**: Usuário pode alternar idioma em `instance/settings` via radio (pt-BR / en-US)
...
- [x] **SETTINGS-04**: Mudança de idioma aplica imediatamente na sessão atual sem reload completo da página
...
- [x] **UI-01**: Tela inbox traduzida (lista de items, filtros, ações em massa, estados vazios)
...
- [x] **UI-04**: Telas admin/company traduzidas (membros, roles, claude accounts pool, cost summary, rotation history)
- [x] **UI-05**: Navegação, sidebar, header, menus e breadcrumbs traduzidos
- [x] **UI-06**: Formulários de auth (login, signup, reset password, invite/board-claim) traduzidos
- [x] **UI-07**: Mensagens de erro, validação de formulários e mensagens de API traduzidas (server emit shape ✓ via 09-03a; client mapping ✓ via 09-03b)
- [x] **UI-08**: Tooltips, empty states, modais de confirmação e toasts traduzidos
...
- [x] **AGENT-MSG-02**: Summaries e relatórios gerados por agentes apresentados em pt-BR quando idioma ativo
- [x] **AGENT-MSG-03**: Prompts UI dos agentes (botões, headers, labels de painéis de agente) traduzidos
- [x] **AGENT-MSG-04**: Notificações e toasts de eventos de agente em pt-BR
...
- [x] **AGENT-SKILL-02**: Skills/templates de prompt do paperclip que falam com o usuário traduzidos para pt-BR
...
- [x] **AGENT-SKILL-04**: Comportamento empiricamente validado — agente realmente responde em pt-BR após troca de idioma (HUMAN-UAT)
...
| AGENT-SKILL-02 | Fase 11 | Complete |
| AGENT-SKILL-03 | Fase 11 | Complete |
| AGENT-SKILL-04 | Fase 11 | Complete |
