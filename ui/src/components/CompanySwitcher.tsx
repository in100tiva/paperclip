  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
        ...
        >
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-sm font-medium truncate">
              {selectedCompany?.name ?? t("common:nav.company-switcher.select-placeholder")}
            </span>
          </div>
      ...
      <DropdownMenuContent align="start" className="w-[220px]">
        <DropdownMenuLabel>{t("common:nav.company-switcher.companies-label")}</DropdownMenuLabel>
        <DropdownMenuSeparator />
        ...
        {sidebarCompanies.length === 0 && (
          <DropdownMenuItem disabled>{t("common:nav.company-switcher.no-companies")}</DropdownMenuItem>
        )}
        <DropdownMenuSeparator />
        <DropdownMenuItem asChild>
          <Link to="/company/settings" className="no-underline text-inherit">
            <Settings className="h-4 w-4 mr-2" />
            {t("common:nav.company-switcher.company-settings")}
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild>
          <Link to="/companies" className="no-underline text-inherit">
            <Plus className="h-4 w-4 mr-2" />
            {t("common:nav.company-switcher.manage-companies")}
          </Link>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
