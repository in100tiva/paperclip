...
export function ActiveAgentsPanel({
  ...
  const resolvedTitle = title ?? t("agents:panel.active-agents");
  const resolvedEmptyMessage = emptyMessage ?? t("agents:panel.no-recent-runs");
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>{isActive ? t("agents:panel.live-now") : run.finishedAt ? t("agents:panel.finished-relative", { time: relativeTime(run.finishedAt) }) : t("agents:panel.started-relative", { time: relativeTime(run.createdAt) })}</span>
            </div>
          </div>
