...
// Pitfall 1 (RESEARCH §Common Pitfalls): keep only values as a const tuple,
// resolve labels at render-time via t() so missing-keys regex picks up the call.
...
// Static map from db enum values to kebab-case JSON keys.
// Keeps narrow literal types so i18next typed `t()` accepts the keys.
...
export function NewProjectDialog() {
  ...
  const uploadDescriptionImage = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) throw new Error(t("projects:errors.no-company-selected"));
  ...
  const deriveWorkspaceNameFromPath = (value: string) => {
    ...
    return segments[segments.length - 1] ?? t("projects:labels.local-folder");
  };
  ...
  return (
    ...
    >
      ...
      >
        <DialogTitle className="sr-only">{t("projects:new-project.title")}</DialogTitle>
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{t("projects:new-project.title")}</span>
          </div>
        ...
        <div className="px-4 pb-2">
          <MarkdownEditor
            ...
            placeholder={t("projects:new-project.description-placeholder")}
            bordered={false}
        ...
        <div className="px-4 pt-3 pb-3 space-y-3 border-t border-border">
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("projects:new-project.repo-url-label")}</label>
              <span className="text-xs text-muted-foreground/50">{t("projects:new-project.optional")}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                </TooltipTrigger>
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("projects:new-project.tooltip-repo")}
                </TooltipContent>
              </Tooltip>
            </div>
            <input
          ...
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("projects:new-project.local-folder-label")}</label>
              <span className="text-xs text-muted-foreground/50">{t("projects:new-project.optional")}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("projects:new-project.tooltip-local-folder")}
                </TooltipContent>
              </Tooltip>
            </div>
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap">
          ...
          <Popover open={goalOpen} onOpenChange={setGoalOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {selectedGoals.length > 0 ? t("projects:new-project.goal-add") : t("projects:new-project.goal")}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-56 p-1" align="start">
              {selectedGoals.length === 0 && (
                ...
                >
                  {t("projects:new-project.goal-none")}
                </button>
              )}
              ...
              {selectedGoals.length > 0 && availableGoals.length === 0 && (
                <div className="px-2 py-1.5 text-xs text-muted-foreground">
                  {t("projects:new-project.goal-all-selected")}
                </div>
              )}
            </PopoverContent>
          </Popover>
          ...
          <div className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs">
            ...
            <input
              ...
              placeholder={t("projects:new-project.target-date-placeholder")}
            />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{t("projects:actions.create-failed")}</p>
          ) : (
            <span />
          )}
          <Button
          ...
          >
            {createProject.isPending ? t("projects:actions.creating") : t("projects:actions.create")}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
