...
function ImportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("settings:company-import.select-file-preview")} />
    );
  }
      ...
      <div className="min-h-[560px] px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
            {t("settings:company-import.binary-preview-unavailable")}
          </div>
        )}
      </div>
    </div>
      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">
            {t("settings:company-import.renames-section")}
          </h3>
          <span className="text-xs text-muted-foreground">
            {t("settings:company-import.items-count", { count: conflicts.length })}
          </span>
        </div>
          ...
          {conflicts.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isSkipped ? t("settings:company-import.skipped") : t("settings:company-import.skip")}
                </button>
                  ...
                  >
                    {isConfirmed ? (
                      <>
                        ...
                        {t("settings:company-import.confirmed")}
                      </>
                    ) : (
                      t("settings:company-import.confirm-rename")
                    )}
                  </button>
                )}
              </div>
...
function AdapterPickerList({
  ...
  return (
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">{t("settings:company-import.adapters-section")}</h3>
          <span className="text-xs text-muted-foreground">
            {t("settings:company-import.agents-count", { count: agents.length })}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
              ...
              <div key={agent.slug}>
                <div className="flex items-center gap-3 px-4 py-2.5 text-sm">
                  <span className={cn(
                    ...
                    {t("settings:company-import.agent-label")}
                  </span>
                  ...
                  >
                    ...
                    {t("settings:company-import.configure-adapter")}
                  </button>
                </div>
                {isExpanded && (
...
async function readLocalPackageZip(file: File): Promise<{
  ...
  // Note: error messages here propagate to handleChooseLocalPackage's catch
  // and are shown via translateApiError-style fallback. Keeping them in English
  // because they're library-level invariants (consumed by client only via the
  // localized toast title that sits above the body).
...
export function CompanyImport() {
  ...
  const localZipHelpText = t("settings:company-import.local-zip-help");

  useEffect(() => {
    setBreadcrumbs([
      { label: t("settings:org.crumb"), href: "/org" },
      { label: t("settings:company-import.crumb") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const previewMutation = useMutation({
    mutationFn: () => {
      ...
      if (!source) throw new Error(t("settings:company-import.no-source-error"));
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("settings:company-import.preview-failed-title"),
        body: err instanceof Error ? err.message : t("settings:company-import.preview-failed-body"),
      });
    },
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("settings:company-import.import-complete-title"),
        body: t("settings:company-import.import-complete-body", { name: result.company.name, count: result.agents.length }),
      });
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("settings:company-import.import-failed-title"),
        body: err instanceof Error ? err.message : t("settings:company-import.import-failed-body"),
      });
    },
  });
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      pushToast({
        tone: "error",
        title: t("settings:company-import.package-read-failed-title"),
        body: err instanceof Error ? err.message : t("settings:company-import.package-read-failed-body"),
      });
    }
  }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message={t("settings:company-import.select-company")} />;
  }

  const sourceModeOptions = [
    { key: "github" as const, icon: Github, label: t("settings:company-import.source-github") },
    { key: "local" as const, icon: Upload, label: t("settings:company-import.source-local") },
  ];
  ...
  return (
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          <h2 className="text-base font-semibold">{t("settings:company-import.import-source-section")}</h2>
          <p className="text-xs text-muted-foreground mt-1">
            {t("settings:company-import.import-source-description")}
          </p>
        </div>

        <div className="grid gap-2 md:grid-cols-2">
          {sourceModeOptions.map(({ key, icon: Icon, label }) => (
            <button
              key={key}
              type="button"
              className={cn(
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {t("settings:company-import.choose-zip")}
              </Button>
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {t("settings:company-import.package-summary", { name: localPackage.name, count: Object.keys(localPackage.files).length })}
                </span>
              )}
            </div>
            {!localPackage && (
          ...
          <Field
            label={t("settings:company-import.github-url-label")}
            hint={t("settings:company-import.github-url-hint")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              value={importUrl}
              placeholder={t("settings:company-import.github-url-placeholder")}
              onChange={(e) => {
        ...
        <Field label={t("settings:company-import.target-label")} hint={t("settings:company-import.target-hint")}>
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
          ...
          >
            <option value="new">{t("settings:company-import.target-new")}</option>
            <option value="existing">
              {t("settings:company-import.target-existing", { name: selectedCompany?.name ?? "" })}
            </option>
          </select>
        </Field>
        ...
        {targetMode === "new" && (
          <Field
            label={t("settings:company-import.new-name-label")}
            hint={t("settings:company-import.new-name-hint")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              ...
              placeholder={t("settings:company-import.new-name-placeholder")}
            />
          </Field>
        )}

        <Field
          label={t("settings:company-import.collision-label")}
          hint={t("settings:company-import.collision-hint")}
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
          ...
          >
            <option value="rename">{t("settings:company-import.collision-rename")}</option>
            <option value="skip">{t("settings:company-import.collision-skip")}</option>
            <option value="replace">{t("settings:company-import.collision-replace")}</option>
          </select>
        </Field>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {previewMutation.isPending
              ? t("settings:company-import.previewing")
              : t("settings:company-import.preview-cta")}
          </Button>
        </div>
      </div>
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              <span className="font-medium">
                {t("settings:company-import.preview-section")}
              </span>
              <span className="text-muted-foreground">
                {t("settings:company-import.files-selected", { selected: selectedCount, total: totalFiles, count: totalFiles })}
              </span>
              {conflicts.length > 0 && (
                <span className="text-amber-500">
                  {t("settings:company-import.conflicts-count", { count: conflicts.length })}
                </span>
              )}
              {importPreview.errors.length > 0 && (
                <span className="text-destructive">
                  {t("settings:company-import.errors-count", { count: importPreview.errors.length })}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="mx-5 mt-3 flex justify-end">
            ...
            >
              ...
              {importMutation.isPending
                ? t("settings:company-import.importing")
                : t("settings:company-import.import-cta", { count: selectedCount })}
            </Button>
          </div>
          ...
          <div className="grid h-[calc(100vh-16rem)] gap-0 xl:grid-cols-[19rem_minmax(0,1fr)]">
            <aside className="flex flex-col border-r border-border overflow-hidden">
              <div className="border-b border-border px-4 py-3 shrink-0">
                <h2 className="text-base font-semibold">{t("settings:company-import.package-files")}</h2>
              </div>
