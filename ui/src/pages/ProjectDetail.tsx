  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        <div>
          <span className="text-muted-foreground">{t("projects:labels.status")}</span>
          <div className="mt-1">
        ...
        {project.targetDate && (
          <div>
            <span className="text-muted-foreground">{t("projects:labels.target-date")}</span>
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        aria-label={t("projects:color-picker.change-aria")}
      />
      {open && (
          ...
          <div className="grid grid-cols-5 gap-1.5">
            {PROJECT_COLORS.map((color) => (
              <button
                ...
                aria-label={t("projects:color-picker.select-aria", { color })}
              />
            ))}
          </div>
        </div>
...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onSuccess: (updatedProject, archived) => {
      invalidateProject();
      const name = updatedProject?.name ?? project?.name ?? t("projects:default-name");
      if (archived) {
        pushToast({ title: t("projects:toast.archived", { name }), tone: "success" });
        navigate("/dashboard");
      } else {
        pushToast({ title: t("projects:toast.unarchived", { name }), tone: "success" });
      }
    },
    onError: (_, archived) => {
      pushToast({
        title: archived ? t("projects:toast.archive-failed") : t("projects:toast.unarchive-failed"),
        tone: "error",
      });
    },
  });
  ...
  const uploadImage = useMutation({
    mutationFn: async (file: File) => {
      if (!resolvedCompanyId) throw new Error(t("projects:errors.no-company-selected"));
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: project?.name ?? routeProjectRef ?? t("projects:default-name") },
    ]);
  ...
  const projectBudgetSummary = useMemo(() => {
    ...
    return {
      ...
      scopeName: project?.name ?? t("projects:default-name"),
      metric: "billed_cents",
      windowKind: "lifetime",
      amount: 0,
      observedAmount: 0,
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab ?? "list"} onValueChange={(value) => handleTabChange(value as ProjectTab)}>
        <PageTabBar
          items={[
            { value: "list", label: t("projects:tabs.issues") },
            { value: "overview", label: t("projects:tabs.overview") },
            ...(showWorkspacesTab ? [{ value: "workspaces", label: t("projects:tabs.workspaces") }] : []),
            { value: "configuration", label: t("projects:tabs.configuration") },
            { value: "budget", label: t("projects:tabs.budget") },
      ...
      {activeTab === "workspaces" ? (
        workspaceTabDecisionLoaded ? (
          ...
          <p className="text-sm text-muted-foreground">{t("projects:loading.workspaces")}</p>
        )
      ) : null}
