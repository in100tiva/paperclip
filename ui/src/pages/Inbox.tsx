...
type JoinRequestLabelInput = Pick<
  JoinRequest,
...
// Loose translate signature so callers can pass either react-i18next's `t` or any compatible function.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type JoinRequestLabelTranslate = (key: any, options?: any) => string;

const fallbackJoinRequestTranslate: JoinRequestLabelTranslate = (key, options) => {
  if (key === "inbox:join-request.agent-prefix") return "Agent join request";
  if (key === "inbox:join-request.agent-with-name") return `Agent join request: ${options?.name ?? ""}`;
  if (key === "inbox:join-request.human-fallback") return "Human join request";
  return String(key);
};

export function formatJoinRequestInboxLabel(
  joinRequest: JoinRequestLabelInput,
  translate?: JoinRequestLabelTranslate,
) {
  const tt: JoinRequestLabelTranslate = translate ?? fallbackJoinRequestTranslate;

  if (joinRequest.requestType !== "human") {
    return joinRequest.agentName
      ? tt("inbox:join-request.agent-with-name", { name: joinRequest.agentName })
      : tt("inbox:join-request.agent-prefix");
  }
  ...
  return tt("inbox:join-request.human-fallback");
}
...
export function FailedRunInboxRow({
  ...
  const displayError = runFailureMessage(run, t("inbox:failed-run.exit-error-fallback"));
      ...
      <div className="flex items-start gap-2 sm:items-center">
          ...
          <span className="hidden sm:inline-flex h-4 w-4 shrink-0 items-center justify-center self-center">
            {showUnreadDot ? (
              <button
                ...
                aria-label={t("inbox:actions.mark-as-read")}
              >
                <span className={cn(
              ...
              <button
                ...
                aria-label={t("inbox:actions.dismiss-from-inbox")}
              >
          ...
          <span className="min-w-0 flex-1">
            <span className="line-clamp-2 text-sm font-medium sm:truncate sm:line-clamp-none">
              {issue ? (
                ...
                <>{linkedAgentName ? t("inbox:failed-run.label-with-agent", { agentName: linkedAgentName }) : t("inbox:failed-run.label")}</>
              )}
            </span>
        ...
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
          ...
          >
            ...
            {isRetrying ? t("inbox:actions.retrying") : t("inbox:actions.retry")}
          </Button>
          {!showUnreadSlot && (
            <button
              type="button"
              ...
              aria-label={t("inbox:actions.dismiss")}
            >
              <X className="h-4 w-4" />
            </button>
          )}
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              ...
              {requesterName ? <span>{t("inbox:approval.requested-by", { name: requesterName })}</span> : null}
              <span>{t("inbox:approval.updated-when", { when: timeAgo(approval.updatedAt) })}</span>
            </span>
          </span>
        </Link>
      ...
      <div className="flex items-start gap-2 sm:items-center">
        ...
        <div className="flex min-w-0 flex-1 items-start gap-2">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              <span>{t("inbox:join-request.requested-line", { when: timeAgo(joinRequest.createdAt), ip: joinRequest.requestIp })}</span>
              {joinRequest.adapterType && <span>{t("inbox:join-request.adapter-line", { adapter: joinRequest.adapterType })}</span>}
            </span>
          </span>
        </div>
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
...
export function Inbox() {
  ...
  const filteredWorkItems = useMemo(() => {
    ...
    return workItemsToRender.filter((item) => {
      ...
      if (item.kind === "failed_run") {
        ...
        const msg = runFailureMessage(run, t("inbox:failed-run.exit-error-fallback"));
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox:errors.approve-failed"));
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox:errors.reject-failed"));
    },
  });
  ...
  const approveJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox:errors.approve-join-failed"));
    },
  });
  ...
  const rejectJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox:errors.reject-join-failed"));
    },
  });
  ...
  const retryRunMutation = useMutation({
    mutationFn: async (run: HeartbeatRun) => {
      ...
      if (!("id" in result)) {
        throw new Error(result.message ?? t("inbox:errors.retry-skipped"));
      }
  ...
  const archiveIssueMutation = useMutation({
    ...
    onError: (err, id, context) => {
      setActionError(err instanceof Error ? err.message : t("inbox:errors.archive-failed"));
  ...
  const unarchiveIssueMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox:errors.undo-archive-failed"));
    },
    onSuccess: (_data, id) => {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={InboxIcon} message={t("inbox:empty-state.select-company")} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            title={nestingEnabled ? t("inbox:toolbar.nesting-disable") : t("inbox:toolbar.nesting-enable")}
          >
          ...
          <Popover>
            ...
            <PopoverContent align="end" className="w-40 p-2">
              <div className="space-y-0.5">
                {([
                  ["none", t("inbox:toolbar.group-none")],
                  ["type", t("inbox:toolbar.group-type")],
                  ...(isolatedWorkspacesEnabled ? ([["workspace", t("inbox:toolbar.group-workspace")]] as const) : []),
          ...
          <IssueColumnPicker
            ...
            title={t("inbox:toolbar.columns-title")}
            iconOnly
          />
          {canMarkAllRead && (
            <>
              ...
              >
                {markAllReadMutation.isPending ? t("inbox:actions.marking") : t("inbox:actions.mark-all-as-read")}
              </Button>
              <Dialog open={showMarkAllReadConfirm} onOpenChange={setShowMarkAllReadConfirm}>
                <DialogContent className="sm:max-w-md">
                  <DialogHeader>
                    <DialogTitle>{t("inbox:actions.mark-all-as-read-confirm-title")}</DialogTitle>
                    <DialogDescription>
                      {t("inbox:actions.mark-all-as-read-confirm-description", { count: unreadIssueIds.length })}
                    </DialogDescription>
                  </DialogHeader>
                  <DialogFooter>
                    ...
                    >
                      {t("inbox:actions.mark-all-as-read")}
                    </Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <SelectTrigger className="h-8 w-[170px] text-xs">
              <SelectValue placeholder={t("inbox:categories.trigger-placeholder")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="everything">{t("inbox:categories.everything")}</SelectItem>
              <SelectItem value="issues_i_touched">{t("inbox:categories.issues-i-touched")}</SelectItem>
              <SelectItem value="join_requests">{t("inbox:categories.join-requests")}</SelectItem>
              <SelectItem value="approvals">{t("inbox:categories.approvals")}</SelectItem>
              <SelectItem value="failed_runs">{t("inbox:categories.failed-runs")}</SelectItem>
          ...
          {showApprovalsCategory && (
            ...
            >
              <SelectTrigger className="h-8 w-[170px] text-xs">
                <SelectValue placeholder={t("inbox:approval-status.trigger-placeholder")} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t("inbox:approval-status.all")}</SelectItem>
                <SelectItem value="actionable">{t("inbox:approval-status.actionable")}</SelectItem>
                <SelectItem value="resolved">{t("inbox:approval-status.resolved")}</SelectItem>
              </SelectContent>
            </Select>
          )}
        </div>
      ...
      {allLoaded && visibleSections.length === 0 && (
        <EmptyState
          ...
          message={
            searchQuery.trim()
              ? t("inbox:empty-state.no-search-results")
              : tab === "mine"
              ? t("inbox:empty-state.inbox-zero")
              : tab === "unread"
              ? t("inbox:empty-state.no-new-items")
              : tab === "recent"
                ? t("inbox:empty-state.no-recent-items")
                : t("inbox:empty-state.no-items-match-filters")
          }
        />
      )}
                    ...
                    elements.push(
                      ...
                      >
                        ...
                        <span className="shrink-0 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                          {group.searchSection === "archived" ? t("inbox:groups.archived") : t("inbox:groups.other-results")}
                        </span>
                        <div className="h-px flex-1 bg-border/80" />
                      </div>,
                    );
                  ...
                  >
                    ...
                    <span className="text-sm">
                      {dashboard!.agents.error === 1
                        ? t("inbox:alerts.agent-error-singular", { count: dashboard!.agents.error })
                        : t("inbox:alerts.agent-error-plural", { count: dashboard!.agents.error })}
                    </span>
                  </Link>
                  <button
                    type="button"
              ...
              {showBudgetAlert && (
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      {t("inbox:alerts.budget-utilization", { percent: dashboard!.costs.monthUtilizationPercent })}
                    </span>
                  </Link>
                  <button
                    type="button"
