...
// Permissive TFunction shape — accepts any namespace tuple. Mirrors translateApiError pattern.
...
function mapInviteAuthFeedback(
  ...
  // Cast to permissive signature so we can pass dynamic namespaced keys.
  const tt = t as unknown as (key: string, options?: Record<string, unknown>) => string;
  const code = getAuthErrorCode(error);
  const message = getAuthErrorMessage(error);
  const emailLabel = email.trim().length > 0 ? email.trim() : tt("auth:invite.auth.fallback");

  if (code === "USER_ALREADY_EXISTS_USE_ANOTHER_EMAIL") {
    return {
      tone: "info",
      message: tt("auth:invite.auth.user-already-exists-info", { email: emailLabel }),
    };
  }
  ...
  if (code === "INVALID_EMAIL_OR_PASSWORD") {
    return {
      tone: "error",
      message: tt("auth:invite.auth.invalid-email-or-password"),
    };
  }
  ...
  if (authMode === "sign_in" && message === "Request failed: 401") {
    return {
      tone: "error",
      message: tt("auth:invite.auth.request-failed-401"),
    };
  }
  ...
  if (authMode === "sign_up" && message === "Request failed: 422") {
    return {
      tone: "info",
      message: tt("auth:invite.auth.request-failed-422", { email: emailLabel }),
    };
  }

  return {
    tone: "error",
    message: message ?? tt("auth:invite.auth.fallback"),
  };
}
...
function AwaitingJoinApprovalPanel({
  ...
  const approverLabel = invitedByUserName ?? t("auth:invite.awaiting-approval.approver-fallback");
      ...
      <div className="mx-auto max-w-md border border-zinc-800 bg-zinc-950 p-6" data-testid="invite-pending-approval">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold">
            {t("auth:invite.awaiting-approval.title", { companyName: companyDisplayName })}
          </h1>
        </div>
        <div className="mt-4 space-y-3">
          <p className="text-sm text-zinc-400">
            {t("auth:invite.awaiting-approval.description", { approver: approverLabel })}
          </p>
            ...
            <Trans
              ...
              components={{
                linkTo: (
                  <a
                    href={approvalUrl}
                    className="text-zinc-200 underline underline-offset-2 hover:text-zinc-100"
                  />
                ),
              }}
            />
          ...
          <p className="text-xs text-zinc-500">
            {t("auth:invite.awaiting-approval.refresh-hint")}
          </p>
        </div>
...
export function InviteLandingPage() {
  ...
  const companyDisplayName = companyName || t("auth:invite.company-fallback");
  ...
  const acceptMutation = useMutation({
    mutationFn: async () => {
      if (!invite) throw new Error(t("auth:invite.invite-not-found"));
      if (isCheckingExistingMembership) {
        throw new Error(t("auth:invite.checking-access-error"));
      }
      if (isCurrentMember) {
        throw new Error(t("auth:invite.already-belongs"));
      }
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : t("auth:invite.accept-failed-fallback"));
    },
  });
  ...
  const joinButtonLabel = useMemo(() => {
    if (!invite) return t("auth:invite.continue-cta");
    if (invite.inviteType === "bootstrap_ceo") return t("auth:invite.accept-cta");
    if (showsAgentForm) return t("auth:invite.submit-cta");
    return sessionQuery.data ? t("auth:invite.accept-cta") : t("auth:invite.continue-cta");
  }, [invite, sessionQuery.data, showsAgentForm, t]);

  if (!token) {
    return (
      <div className="mx-auto max-w-xl py-10 text-sm text-destructive">
        {t("auth:invite.invalid-token")}
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="border border-border bg-card p-6" data-testid="invite-error">
          <h1 className="text-lg font-semibold">{t("auth:invite.not-available-title")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("auth:invite.not-available-description")}
          </p>
        </div>
      </div>
    );
        ...
        <div className="border border-border bg-card p-6" data-testid="invite-error">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {inviteJoinRequestStatus === "rejected"
              ? t("auth:invite.rejected-description")
              : t("auth:invite.already-used-description")}
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-zinc-950 px-6 py-12 text-zinc-100">
        <div className="mx-auto max-w-md border border-zinc-800 bg-zinc-950 p-6">
          <h1 className="text-lg font-semibold">{t("auth:invite.bootstrap-complete-title")}</h1>
          <div className="mt-4">
          ...
          <div className="mx-auto max-w-md border border-zinc-800 bg-zinc-950 p-6">
            <div className="flex items-center gap-3">
              <InviteCompanyLogo
                ...
                className="h-12 w-12 border border-zinc-800 rounded-none"
              />
              <h1 className="text-lg font-semibold">{t("auth:invite.joined-title")}</h1>
            </div>
            <div className="mt-4">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                <p className="text-xs uppercase tracking-[0.24em] text-zinc-500">
                  {t("auth:invite.header-eyebrow")}
                </p>
                <h1 className="mt-2 text-2xl font-semibold">
                  {invite.inviteType === "bootstrap_ceo"
                    ? t("auth:invite.header-bootstrap")
                    : t("auth:invite.header-join", { companyName: companyDisplayName })}
                </h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-300">
                  {showsAgentForm
                    ? t("auth:invite.description-agent")
                    : requiresHumanAccount
                      ? t("auth:invite.description-human-needs-account")
                      : t("auth:invite.description-ready")}
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-[0.2em] text-zinc-500">
                  {t("auth:invite.field.company")}
                </div>
                <div className="mt-1 text-sm text-zinc-100">{companyDisplayName}</div>
              </div>
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-[0.2em] text-zinc-500">
                  {t("auth:invite.field.invited-by")}
                </div>
                <div className="mt-1 text-sm text-zinc-100">
                  {invitedByUserName ?? t("auth:invite.invited-by-fallback")}
                </div>
              </div>
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-[0.2em] text-zinc-500">
                  {t("auth:invite.field.requested-access")}
                </div>
                <div className="mt-1 text-sm text-zinc-100">
                  {showsAgentForm
                    ? t("auth:invite.access-agent")
                    : requestedHumanRole ?? t("auth:invite.access-company")}
                </div>
              </div>
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-[0.2em] text-zinc-500">
                  {t("auth:invite.field.expires")}
                </div>
          ...
          <section className={`${panelClassName} h-fit`}>
            {showsAgentForm ? (
              <div className="space-y-4">
                <div>
                  <h2 className="text-lg font-semibold">{t("auth:invite.agent-form-title")}</h2>
                  <p className="mt-1 text-sm text-zinc-400">
                    {t("auth:invite.agent-form-description", { companyName: companyDisplayName })}
                  </p>
                </div>
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">{t("auth:invite.field.agent-name")}</span>
                  <input
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">{t("auth:invite.field.adapter-type")}</span>
                  <select
                    className={fieldClassName}
                    value={adapterType}
                  ...
                  >
                    {joinAdapterOptions.map((type) => (
                      <option key={type} value={type} disabled={!ENABLED_INVITE_ADAPTERS.has(type)}>
                        ...
                        {!ENABLED_INVITE_ADAPTERS.has(type) ? t("auth:invite.coming-soon-suffix") : ""}
                      </option>
                    ))}
                  </select>
                </label>
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">{t("auth:invite.field.capabilities")}</span>
                  <textarea
                    className={fieldClassName}
                    rows={4}
                    value={capabilities}
                ...
                >
                  {acceptMutation.isPending ? t("auth:common.working") : joinButtonLabel}
                </Button>
              </div>
            ) : requiresHumanAccount ? (
              <div className="space-y-5">
                <div>
                  <h2 className="text-lg font-semibold">
                    {authMode === "sign_up"
                      ? t("auth:invite.create-account-title")
                      : t("auth:invite.sign-in-title")}
                  </h2>
                  <p className="mt-1 text-sm text-zinc-400">
                    {authMode === "sign_up"
                      ? t("auth:invite.create-account-description", { companyName: companyDisplayName })
                      : t("auth:invite.sign-in-description")}
                  </p>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {t("auth:invite.mode-create-account")}
                  </button>
                  <button
                    type="button"
                    className={`${modeButtonBaseClassName} ${
                  ...
                  >
                    {t("auth:invite.mode-have-account")}
                  </button>
                </div>
                ...
                <form
                  ...
                  onSubmit={(event) => {
                    ...
                    if (!authCanSubmit) {
                      setAuthFeedback({
                        tone: "error",
                        message: t("errors:validation.required-fields"),
                      });
                      return;
                    }
                    authMutation.mutate();
                ...
                >
                  {authMode === "sign_up" ? (
                    <label className="block text-sm">
                      <span className="mb-1 block text-zinc-400">{t("auth:common.name")}</span>
                      <input
                        name="name"
                        className={fieldClassName}
                        value={name}
                  ...
                  <label className="block text-sm">
                    <span className="mb-1 block text-zinc-400">{t("auth:common.email")}</span>
                    <input
                      name="email"
                      type="email"
                      className={fieldClassName}
                  ...
                  <label className="block text-sm">
                    <span className="mb-1 block text-zinc-400">{t("auth:common.password")}</span>
                    <input
                      name="password"
                      type="password"
                      className={fieldClassName}
                  ...
                  >
                    {authMutation.isPending
                      ...
                      : authMode === "sign_in"
                        ? t("auth:invite.submit-sign-in")
                        : t("auth:invite.submit-create")}
                  </Button>
                </form>

                <p className="text-xs leading-5 text-zinc-500">
                  {authMode === "sign_up"
                    ? t("auth:invite.tail-copy-create")
                    : t("auth:invite.tail-copy-sign-in")}
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                <div>
                  <h2 className="text-lg font-semibold">
                    {shouldAutoAcceptHumanInvite
                      ? t("auth:invite.auto-accept-title")
                      : invite.inviteType === "bootstrap_ceo"
                        ? t("auth:invite.accept-bootstrap-title")
                        : t("auth:invite.accept-company-title")}
                  </h2>
                  <p className="mt-1 text-sm text-zinc-400">
                    {shouldAutoAcceptHumanInvite
                      ? t("auth:invite.auto-accept-description", { companyName: companyDisplayName })
                      : isCurrentMember
                        ? t("auth:invite.already-member", { companyName: companyDisplayName })
                        : invite.inviteType === "bootstrap_ceo"
                          ? `${t("auth:invite.this-will-bootstrap")}.`
                          : `${t("auth:invite.this-will-join", { companyName: companyDisplayName })}.`}
                  </p>
                </div>
                {error ? <p className="text-xs text-red-400">{error}</p> : null}
                {shouldAutoAcceptHumanInvite ? (
                  <div className="text-sm text-zinc-400">
                    {acceptMutation.isPending
                      ? t("auth:invite.submitting-request")
                      : t("auth:invite.finishing-signin")}
                  </div>
                ) : (
                  <Button
                    className="w-full rounded-none"
